...
int leader_chosing(int rank, int nProcesses) {
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for(int i = 0; i < num_neigh; i++) {
			int r = MPI_Send(&leader, 1, MPI_INT, neigh[i], 0, MPI_COMM_WORLD);
			if(r != 0) {
				printf("I can't send data. I'm process %d\n", rank);
			}

			r = MPI_Recv(&leader_vec, 1, MPI_INT, neigh[i], 0, MPI_COMM_WORLD, &status);
			if(r != 0) {
				printf("I can't send data. I'm process %d\n", rank);
			}
